...
akks <- akks0001 %>%
  mutate(Akk_abundance = rowSums(.),
         ...
         has_AmII = ifelse(ASV2 >= 0.0001, "AmI_Positive", "AmI_Negative"),
         has_AmIII = ifelse(ASV4 >= 0.0001, "AmI_Positive", "AmI_Negative"),
         has_AmIV = ifelse(ASV3 >= 0.0001, "AmI_Positive", "AmI_Negative"),
...
```{r}
saveRDS(rel.ps, file.path(out.dir, 'delacuestazuluage_wPhylogroups.rds'))
```
