...
Just use forward reads
```{r}
filt.out <- filterAndTrim(fnFs, filtFs, trimLeft = 10, truncLen = 180,
...
Just reverse reads
```{r}
filt.out <- filterAndTrim(fnRs, filtRs, trimLeft = 10, truncLen = 220,
                          maxN = 0, maxEE = 2, truncQ = 2, rm.phix = TRUE,
                          compress = TRUE, multithread = FALSE)
```
...
#Learn Error Rates
```{r}
errF <- learnErrors(filtFs, mutlithread = TRUE)
errR <- learnErrors(filtRs, mutlithread = TRUE)
```
...
track %>%
  ...
  ggplot(mapping = aes(x = staged, y = counts, by = sample, group = sample)) + geom_lines(alpha = 0.05) + theme_classic()
```
...
ps <- phyloseq(otus, sd, tab_table(taxa))
