...
#Define Paths
```{r}
matson.rds <- "../../Data/Analysis/matson.rds"
out.dir <- "../../Data/Analysis"
```

Load and check phyloseq object
```{r}
matson.ps = read_rds(matson.rds)
print(matson.ps)
```

```{r}
sample_variables(matson.ps)
```
...
Taking the sequences of the three and comparing to known sequences reveals that ASV1 is an AmII, ASV2 is an AmI, and ASV3 is an AmIV.
...
akks <- akks0001 %>%
  mutate(Akk_abundance = rowSums(.),
         ...
         has_AmII = ifelse(ASV1 >= 0.0001, "AmII_Positive", "AmII_Negative"),
...
sample_data(rel.ps)$AmII_abund <- akks$ASV1
...
```{r}
saveRDS(rel.ps, file.path(out.dir, 'matson_wPhylogroups.rds'))
```

Export for visualization
```{r}
write.csv(sample_data(rel.ps), file.path(out.dir, 'matson_phylogroup_dataframe.csv'))
```
